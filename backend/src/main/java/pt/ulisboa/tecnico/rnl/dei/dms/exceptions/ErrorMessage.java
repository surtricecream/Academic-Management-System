package pt.ulisboa.tecnico.rnl.dei.dms.exceptions;

public enum ErrorMessage {

	NO_SUCH_PERSON("Não existe nenhuma pessoa com o ID %s", 1001),
	NO_SUCH_COURSE("Não existe nenhum curso com o ID %s", 1002),
	NO_SUCH_UC("Não existe nenhuma UC com o ID %s", 1003),
	NO_SUCH_MEMBERSHIP("A pessoa com ID %s não é membro desta UC.", 1010),
	NO_SUCH_TEST("Não existe nenhum teste com o ID %s", 1011),
	NO_SUCH_PROJECT("Não existe nenhum projeto com o ID %s", 1012),
	NO_SUCH_GROUP("Não exites nenhum grupo com o ID %s", 1016),
	NO_SUCH_REVIEW_REQUEST("Não existe nenhum pedido de revisão com o ID %s", 1025),
	NO_STUDENTS_IN_UC("Não existe nenhuma pessoa inscrita nesta UC", 1018),
	PERSON_NAME_NOT_VALID("O nome da pessoa especificado não é válido.", 1004),
	PERSON_ALREADY_EXISTS("Já existe uma pessoa com o ID %s", 1005),
	INVALID_CREDENTIALS("Email ou password inválidos.", 1006),
	INVALID_REGENTE("A pessoa especificada não é um professor regente.", 1007),
	INVALID_GROUP_SIZE("Tamanho de grupo inválido", 1013),
	INVALID_GRADE_SUBJECT("Deve ser fornecido personId ou groupId, mas não ambos", 1020),
	INVALID_SCORE("Nota atribuída inválida", 1021),
	INVALID_GROUP("O grupo não pertence a este projeto", 1022),
	INDIVIDUAL_GRADE_NOT_ALLOWED("Não é permitido atribuir nota individual a um projeto de grupo", 1023),
	INVALID_DEADLINE("Prazo limite inválido", 1026),
	INVALID_JUSTIFICATION("Justificação inválida", 1027),
	INVALID_DECISION("Decisão inválida", 1030),
	INVALID_PERSON_DATA("Dados de pessoa inválidos: %s", 1032),
	INVALID_COURSE_DATA("Dados de curso inválidos: %s", 1033),
	INVALID_UC_DATA("Dados de Uc inválidos: %s", 1034),
	INVALID_TEST_DATA("Dados de Teste inválidos: %s", 1039),
	INVALID_PROJECT_DATA("Dados de Projeto inválidos: %s", 1041),
	NOT_AUTHORIZED("Não está autorizado a executar esta operação.", 1008),
	MEMBERSHIP_ALREADY_EXISTS("A pessoa %s já é membro desta UC.", 1009),
	IST_ID_ALREADY_EXISTS("Já existe uma pessoa com o IST ID %s", 1035),
	EMAIL_ALREADY_EXISTS("Já existe uma pessoa com o email %s", 1036),
	COURSE_CODE_ALREADY_EXISTS("Já existe um curso com o código %s", 1037),
	UC_CODE_ALREADY_EXISTS("Já existe uma UC com o código %s", 1038),
	TEST_TITLE_ALREADY_EXISTS("Já existe um teste com o título %s nesta UC", 1040),
	PROJECT_TITLE_ALREADY_EXISTS("Já existe um projeto com o título %s nesta UC", 1042),
	GROUP_NOT_ALLOWED("Este projeto não permite grupos", 1014),
	GROUP_SIZE_EXCEEDED("O grupo excede o tamanho máximo permitido.", 1015),
	GROUPS_ALREADY_EXIST("Os grupos para este projeto já estão feitos", 1019),
	REVIEW_REQUEST_ALREADY_EXISTS("Já existe um pedido de revisão", 1028),
	REQUEST_ALREADY_REVIEWED("O pedido de revisão já foi visto", 1029),
	REQUEST_ALREADY_DECIDED("O pedido de revisão já foi decidido", 1031),
	MEMBER_NOT_STUDENT("A pessoa com ID %s não é estudante desta UC.", 1017),
	WEIGHT_BUDGET_EXCEEDED("O peso excede o disponível. Peso restante: %s", 1024);

	private final String label;
	private final int code;

	ErrorMessage(String label, int code) {
		this.label = label;
		this.code = code;
	}

	public String getLabel() {
		return this.label;
	}

	public int getCode() {
		return this.code;
	}
}
